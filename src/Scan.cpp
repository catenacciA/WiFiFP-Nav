...
Scan::Scan(const std::string& interface)
    : interface(interface) {
    if (interface.empty()) {
...
bool Scan::scan(std::vector<std::vector<WiFiScanData>>& wifiScanDataVec, bool filter) {
    ...
    for (int scanIdx = 0; scanIdx < numScans; ++scanIdx) {
        ...
        if (!wifi) {
            std::cerr << "Failed to initialize WiFi scan on interface " << interface << std::endl;
            return false;
        }
        ...
        if (numAPs < 0) {
            std::cerr << "Failed to perform WiFi scan on interface " << interface << std::endl;
            return false;
        }
    ...
    for (auto& entry : wifiDataMap) {
        ...
        double sq_sum = std::accumulate(wifiData.begin(), wifiData.end(), 0.0,
    ...
    for (const auto& entry : wifiDataMap) {
        ...
        if (wifiData.size() == 10) {
            for (const auto& data : wifiData) {
                outputFile << data.mac << "," << data.rssi << "," << data.mean << "," << data.std << "," << data.frequency << "\n";
            }
